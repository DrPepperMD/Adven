/*
	...
	And yes I do know this file is not in SRC folder, I will fix that soon.
*/
#include <stdio.h>
#include <stdbool.h>
#include <string.h>
#include <sys\types.h>
#include <sys\stat.h>
#include "lib\misc headers\saveio.h"
#include "lib\misc headers\startscreen.h"
#include "lib\misc headers\player.h"
#include "lib\misc headers\booterror.h"
#include "lib\misc headers\commands.h"
...
MAIN:
	...
	{
		printf("\nDo want to set up a save folder [Y/n]\n> ");
		YES_OR_NO_PROMPT
		if (YES_OR_NO == YES)
		{
			printf("Save folder is going to be located in C:\\Program Files (x86)");
			if (_mkdir("C:\\Program Files (x86)\\adven") == 0)
			{
				printf("Save folder created successfully");

			}
			else
			{
				printf("Save folder could not be created...");
				return 1;
			}
			// fp = fopen("C:\\Users\\%s\\adven");
		}
